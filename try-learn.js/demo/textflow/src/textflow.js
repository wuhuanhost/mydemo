...
var textflow = (function() {
    ...
    //获取txt数据
    tf.getTxtData = function(url) {
        $.get(url, function(data) {
            if (data != null && data != "") {
                console.log(data)
                tf.textAnalysis(data, "textflow");
                tf.pagingProcess();
                tf.page();
            }
        });
    ...
    tf.page = function() {
        ...
        for (var i = 0; i < _data.length; i++) {
            ...
            remark.setAttribute("style", "text-align:center;margin-top:-90px;");
            remark.innerHTML = (i + 1) + "/" + _data.length;
     ...
     * @return {[type]}      [description]
     */
    tf.textAnalysis = function(text, id) {
        //文本流字符串
        this.text = text;
        //文本流容器
        this.tfc = $("#" + id);
        ...
        this.everyBlock = _.split(this.text.replace(tf.rege.matchBlankLines, ""), /\n/g);
    ...
    tf.paging = function(data) {
        ...
        for (var i = 0; i < data.length; i++) {
            ...
            for (var j = 0; j < data[i].length; j++) {
                ...
                if (this.tfc.height() < 600 - 20) {
                    $("#block-" + i).append(p);
                    lineArr.push(data[i][j]);
                    //判断是否到最后一页了
                    if (data[i][j] === "@@@@@@") {
                        // pageList.push(lineArr);
                        tf.pageLists.push(pageList);
                        tf.textFlowData.length = 0;
                        return;
                    }
                } else {
     ...
     * @return {[type]} [description]
     */
    tf.pagingProcess = function() {
        console.log()
        while (tf.textFlowData.length > 0) {
            console.log(tf.textFlowData.length + "???????????????????????????????????")
            tf.paging(tf.textFlowData);
        }
        // console.log("还未分析的数据===================" + tf.textFlowData.length);
        // console.log(tf.textFlowData);
        // console.log("分析完成的数据===================" + tf.pageLists.length);
        // console.log(tf.pageLists);
    }
    return tf;
}());
...
// console.log(textflow.textFlowData);
// console.log("分析完成的数据===================" + textflow.pageLists.length);
// console.log(textflow.pageLists);




console.log(getTxtData("http://localhost:9696/static/txt/test.txt"));
