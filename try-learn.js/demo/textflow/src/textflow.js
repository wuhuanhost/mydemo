...
var textflow = (function() {
    ...
    tf.init = function(text, id) {
        ...
        //textflow对象
        this.textFlowData = [];
        console.log(this.everyBlock.length)
        for (var i = 0; i < this.everyBlock.length; i++) {
            console.log(this.everyBlock[i]);
            tf.index = 0;
            for (var j = 0; j < _.toArray(this.everyBlock[i]).length; j++) {
                if (j == 0) {
                	//转成字符串
                    this.everyLine = _.slice(this.everyBlock[i], tf.index, tf.index + 31);
                    tf.index = tf.index + 31;
                } else {
                    this.everyLine = _.slice(this.everyBlock[i], tf.index, tf.index + 33);
                    tf.index = tf.index + 33;
                }
                if(this.everyLine!=null&&this.everyLine!=""){
					this.textFlowData.push(this.everyLine);
                }
                
            }
        }
        console.log(this.textFlowData.length)
    };

    tf.drawPage = function() {
        var html = "";
        for (var i = 0; i < this.textFlowData.length; i++) {
            html += "<p id='" + i + "'>"
            if (i == 0) {
                html += "&nbsp;&nbsp;";
            }
            for (var j = 0; j < this.textFlowData[i].length; j++) {
                html += "<span>" + this.textFlowData[i][j] + "</span>";
            }
            html += "</p>";
        }
        html += "<hr/>";
