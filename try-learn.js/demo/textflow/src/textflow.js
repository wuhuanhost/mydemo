    ...
    tf.init = function(text, id) {
        ...
        for (var i = 0; i < this.everyBlock.length; i++) {
            ...
            for (var j = 0; j < this.everyBlock[i].length; j++) {
                ...
                if (j === 0) {
                    temp = array2string(_.toArray(this.everyBlock[i]).slice(start, end - 2));
                    sliceIndex = sliceIndex + tf.lineWordSize - 2;
                } else {
                    temp = array2string(_.toArray(this.everyBlock[i]).slice(start, end));
                    _.toString(temp);
    ...
    tf.draw = function() {
        for (var i = 0; i < tf.textFlowData.length; i++) {
            ...
            for (var j = 0; j < tf.textFlowData[i].length; j++) {
                ...
                if (j === 0) {
                    span.innerHTML = "&nbsp;&nbsp;" + tf.textFlowData[i][j];
                } else {
                    span.innerHTML = tf.textFlowData[i][j];
                }
...
textflow.draw();
console.log($("#textflow").scrollTop());
console.log(document.getElementById("textflow").clientHeight);
